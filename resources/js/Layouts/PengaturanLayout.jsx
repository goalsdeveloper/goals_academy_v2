    ...
    return (
        <>
            ...
            <main className="text-dark pt-20 xs:pt-20 md:pt-20 xl:pt-32 3xl:pt-48 pb-[8vw] md:pb-[3vw] text-[3vw] md:text-[1.15vw] overflow-visible">
                <Preliminary auth={auth} title={title} setShowImageUploader={setShowImageUploader} />
                <DashboardContent auth={auth} title={title} setShowImageUploader={setShowImageUploader}>
                    {children}
                </DashboardContent>
                <ImageUploader show={showImageUploader} setShow={setShowImageUploader} profileImage={profileImage} setProfileImage={setProfileImage} />
            </main>
        </>
    )
}

function Preliminary ({ auth, title, setShowImageUploader }) {
    return (
...
function DashboardContent ({ auth, title, setShowImageUploader, children }) {
    return (
        <div className="container mx-auto flex gap-[3vw] md:mt-[3vw]">
            <DashboardNavbar auth={auth} className={`sticky top-24 xs:top-28 md:top-24 xl:top-28 3xl:top-40 z-10 w-full md:w-3/12 h-fit ${title == 'Pengaturan' ? '' : 'hidden md:flex'}`} title={title} setShowImageUploader={setShowImageUploader} />
...
function DashboardNavbar ({ auth, title, setShowImageUploader, className }) {
    return (
        <div className={`flex flex-col gap-[5.5vw] md:gap-[1.5vw] ${className}`}>
            <div className="hidden md:flex items-center gap-[1vw]">
                ...
                <div>
                    ...
                    ><i className="bi bi-pen"></i> Ubah Foto</button>
                </div>
            </div>
        ...
        <>
            ...
            <div
                className={`${
                    show
                        ? "md:top-0 bottom-0"
                        : "md:top-full -bottom-full"
                } fixed left-0 flex flex-col gap-[4vw] md:gap-[1vw] w-full md:w-[30vw] h-[50vh] md:h-[30vw] transition-all duration-500 bg-white shadow-md rounded-t-[6vw] md:rounded-[1vw] p-[8vw] md:p-[1.75vw] z-50 md:ms-[35vw] md:mt-[8vh]`}
            >
                <div className="relative w-full h-full">
                    <Cropper
                        image="https://i.pinimg.com/236x/5e/1b/8c/5e1b8ca405646b34d9dc2eebfba9e4d4.jpg"
                        crop={crop}
                        cropShape="round"
                        showGrid={false}
                        zoom={zoom}
