    ...
    return (
        <>
            ...
            <main className="text-dark pt-20 xs:pt-20 md:pt-20 xl:pt-32 3xl:pt-48 pb-[8vw] md:pb-[3vw] text-[3vw] md:text-[1.15vw] overflow-visible">
                <Preliminary auth={auth} title={title} profileImage={profileImage} setShowImageUploader={setShowImageUploader} />
                <DashboardContent auth={auth} title={title} profileImage={profileImage} setShowImageUploader={setShowImageUploader}>
                    {children}
                </DashboardContent>
...
function DashboardContent ({ auth, title, profileImage, setShowImageUploader, children }) {
    return (
        <div className="container mx-auto flex gap-[3vw] md:mt-[3vw]">
            <DashboardNavbar auth={auth} className={`sticky top-24 xs:top-28 md:top-24 xl:top-28 3xl:top-40 z-10 w-full md:w-3/12 h-fit ${title == 'Pengaturan' ? '' : 'hidden md:flex'}`} title={title} profileImage={profileImage} setShowImageUploader={setShowImageUploader} />
...
function DashboardNavbar ({ auth, title, profileImage, setShowImageUploader, className }) {
    return (
        <div className={`flex flex-col gap-[5.5vw] md:gap-[1.5vw] ${className}`}>
            <div className="hidden md:flex items-center gap-[1vw]">
                <img className="w-[22vw] h-[22vw] md:w-[5vw] md:h-[5vw] rounded-full shadow-centered-spread" src={profileImage ? profileImage : userIcon} alt="User" />
                <div>
        ...
        <>
            ...
            >
                ...
                <div className="flex flex-wrap justify-end gap-[2vw] md:gap-[1vw] mt-[1vw]">
                    ...
                    <input className="hidden" type="file" id="profile_photo" accept="image/*" onChange={onFileChange} />
                    <ButtonPill
                        className="w-3/12"
                        onClick={() => {
                            imageSubmit()
