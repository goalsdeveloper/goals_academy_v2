...
function Preliminary({ auth, title }) {
    const percentage = 60;
    const profileImage = auth.user.profile.profile_image
        ? `/storage/${auth.user.profile.profile_image}`
        : userIcon;

    return (
        <div
            className={`container mx-auto shadow-centered-spread rounded-[1vw] overflow-hidden mt-[3vw] mb-[6vw] md:mb-[3vw] ${
        ...
        >
            <div className="relative flex flex-col md:flex-row text-center md:text-left items-center gap-[3vw] md:gap-[1.5vw] p-[6vw] pb-[12vw] md:p-[3vw]">
                ...
                <div>
                    ...
                    <p>
                        Hello {auth.user.name}! Selamat datang di website Goals
                        Academy!
                    </p>
                </div>
                <img
                    src={wave5}
                    className="hidden md:block absolute w-full left-0 right-0 bottom-0 select-none"
                />
                <img
                    src={wave7}
                    className="md:hidden absolute w-full left-0 right-0 bottom-0 select-none"
                />
            </div>
            <div className="relative bg-secondary text-white">
                <img
                    src={wave6}
                    className="hidden md:block absolute w-full left-0 right-0 top-0 select-none"
                />
                <img
                    src={wave8}
                    className="md:hidden absolute w-full left-0 right-0 top-0 select-none"
                />
                {percentage < 100 ? (
                    <div className="flex flex-wrap md:flex-nowrap justify-center md:justify-normal items-center gap-[4vw] md:gap-[1.5vw] p-[6vw] pt-[12vw] md:p-[3vw]">
                        ...
                        <div className="w-7/12 md:w-8/12 pe-[2vw] md:pe-0">
                            <h2 className="font-sans font-normal md:font-medium text-white text-[4vw] md:text-[2.25vw] mb-[.5vw]">
                                Kelengkapan Profil Anda
                            </h2>
                            <p>
                                Mari lengkapi profil Anda agar Anda dapat segera
                                melakukan transaksi. Profil yang lengkap
                                memudahkan kami memproses pesanan dengan cepat
                                dan aman.
                            </p>
                        </div>
                        <Link
                            href="/pengaturan"
                            className="w-5/12 h-[6vw] md:w-[20%] md:h-[3vw] cursor-pointer"
                        >
...
function DashboardContent({ title, children }) {
    return (
        <div className="container mx-auto flex gap-[1.75vw]">
            ...
            />
            <div
                className={`w-full md:w-9/12 h-fit md:p-[2.5vw] md:shadow-centered-spread md:rounded-xl ${
                    title == "Dashboard" ? "hidden md:block" : ""
                }`}
            >
...
function DashboardNavbar({ title, className }) {
    return (
        <div
            className={`flex flex-col gap-[5.5vw] md:gap-[1.25vw] ${className}`}
        >
            <DashboardNavbarItem
                className={`${
