...
export default function Sidebar ({ items, registrationLink, scrollToSection }) {
    return (
        <nav className="w-[14.3vw] sticky top-[7.5vw] h-fit flex flex-col gap-[1.5vw] font-poppins">
            {items.map((item, index) => (
                <SidebarItem key={index} {...item} onClick={() => scrollToSection(`#${item.href}`)} />
            ))}
            <GoalsButton href={registrationLink} className="w-fit px-[2vw] font-sans text-[1.04vw] rounded-[.5vw]">Daftar Sekarang</GoalsButton>
        </nav>
    )
}
