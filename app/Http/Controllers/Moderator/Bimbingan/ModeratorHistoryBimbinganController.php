    ...
    public function show(Course $history)
    {
        try {
            ...
            if (Auth::user()->user_role == "moderator") {
                $history_user = Course::with('user:id,username', 'user.profile:id,user_id,university,major,phone_number,faculty,rumpun', 'tutor:id,name', 'topic:id,topic', 'place.city', 'order:id,order_code', 'products:id,name,contact_type', 'fileUploads', 'productReview')->findOrFail($history->id);
                // return response()->json([
                //     'status' => true,
                //     'statusCode' => 200,
